...
class AddTicket extends React.Component {
	...
	onSelect = (selectedList, selectedItem) => {
		...
		this.setState({
			employees: selectedList.map((lis) => {
				return { _id: lis._id }
			}),
		})
	}
	...
	render() {
		...
		//console.log(dept)
		const emp = this.props.employees.filter(
			(emp) => emp.department === dept._id
		)
		//console.log(emp)
		return (
			<Container>
				<div>
					<h1 style={{ marginBottom: '40px' }}>Add or Edit Ticket</h1>
...
export default withRouter(connect(mapStateToProps)(AddTicket))
