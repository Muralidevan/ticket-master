...
function TicketShow(props) {
	const cust =
		props.ticket &&
		props.customers.find((cust) => cust._id === props.ticket.customer)
	const dept =
		props.ticket &&
		props.departments.find((dept) => dept._id === props.ticket.department)
	...
	function showEmployees() {
		...
		employees = props.employees.filter(
			(emp) => empId && empId.find((tickemp) => tickemp === emp._id)
		)
