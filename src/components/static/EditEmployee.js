...
class EditEmployee extends React.Component {
	...
	handleSubmit = (e) => {
		...
		const success = () => {
			this.setState({
				success: 'employee details have been submitted successfully',
			})
		}
		const redirect = () => {
			setTimeout(() => {
				this.props.history.push(
					`/employees/${this.props.employee && this.props.employee._id}`
				)
			}, 3000)
		}
		this.props.dispatch(
			startEditEmployee(formData, this.props.employee._id, success, redirect)
		)
	}
	render() {
		return (
